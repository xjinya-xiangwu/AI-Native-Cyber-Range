(() => {
  ...
  const initialState = {
    ...
    speed: 1, assetsOpen: false, playing: false
  };
  ...
  function renderConversation() {
    ...
    return `<article class="message user-message"><div class="avatar human">您</div><div><small>任务目标</small><p>${esc(state.prompt)}</p></div></article>
      <article class="message agent-message"><div class="avatar agent">AI</div><div><small>任务识别 · ${esc(current.type)}</small><p>我会遵循“授权定界 → 环境基线 → 专业执行 → 验证复测 → 清理归档”的网安任务主线，并将每个动作关联到里程碑、工具、观测与可核验运行记录。</p><div class="intent-tags"><span>对象 · ${esc(current.subject)}</span><span>重点 · ${esc(current.priority)}</span><span>环境 · ${esc(current.environment)}</span></div></div></article>
      ${state.phase === 'draft' ? renderClarification() : ''}${creationVisible ? renderCreationStream() : ''}${eventsVisible ? renderExecutionStream() : ''}${state.phase === 'gate-risk' ? renderRiskGate() : ''}${state.phase === 'gate-conflict' ? renderConflictGate() : ''}${state.phase === 'completed' ? renderCompletionMessage() : ''}`;
  }
  ...
  function renderExecutionStream() {
    const current = scenario();
    return `<article class="process-block execution-block"><div class="process-head"><div><span class="process-icon run">◎</span><b>${esc(current.type)}执行记录</b></div><em>${state.phase === 'completed' ? `${current.events.length} 步完成` : `${state.eventIndex + 1} / ${current.events.length}`}</em></div><div class="event-stream">${current.events.slice(0, state.eventIndex + 1).map((event) => `<button class="event-row type-${event.type.toLowerCase()} ${state.selectedEvent === event.id ? 'selected' : ''}" data-event="${event.id}"><span class="event-marker">${event.type === 'ACTION' ? '▶' : event.type === 'QUESTION' ? '!' : event.type === 'RESULT' ? '✓' : '●'}</span><div><div><code>${esc(event.tool)}</code><time>${event.time}</time></div><b>${esc(event.title)}</b><small>${esc(event.detail)}</small></div><em>${esc(event.status)}</em></button>`).join('')}</div></article>`;
  }
  ...
  function renderCompletionMessage() {
    const current = scenario();
    return `<article class="message agent-message final-message"><div class="avatar agent">AI</div><div><small>任务完成 · 产出物卡片</small><h3>${esc(current.type)}任务已完成并完成环境清理</h3><p>${esc(current.events.at(-1).detail)}</p><div class="result-kpis"><span><b>${current.metrics.traces}</b>轨迹事件</span><span><b>${current.metrics.evidence}</b>运行记录</span><span><b>${current.metrics.review}</b>复核项</span><span><b>${current.outputs.length}</b>类产出物</span></div><div class="output-list">${current.outputs.map((item) => `<div><span>✓</span><p><b>${esc(item.type)}</b><small>${esc(item.count)} · ${esc(item.state)}</small></p></div>`).join('')}</div><div class="completion-actions"><button class="primary-button" id="open-assets">打开资产空间</button><button class="ghost-button" id="open-data">查看数据管线</button></div></div></article>`;
  }
  ...
  function renderAssets() {
    const current = scenario(); const drawer = $('#asset-drawer');
    drawer.innerHTML = `<div class="drawer-head"><div><span>资产空间</span><small>按任务归档的报告、记录、场景与数据</small></div><button id="close-assets">×</button></div><div class="drawer-task"><small>来源任务</small><b>${taskId()}</b><span>${esc(phaseLabel())}</span></div><div class="asset-list">${current.assets.map((asset) => `<article class="asset-card"><div><span>${esc(asset.id)}</span><em>${state.phase === 'completed' ? esc(asset.state) : '生成中'}</em></div><h3>${esc(asset.type)}</h3><p>${esc(asset.count)} · ${esc(asset.usage)}</p><small>来源 ${esc(asset.source)}</small></article>`).join('')}</div><div class="drawer-foot">演示数据 · 项目隔离 · 用途授权独立 · 导出受控</div>`;
    $('#close-assets').addEventListener('click', toggleAssets);
  }

  function bindStudio() {
    $('#reset-task').addEventListener('click', resetDemo);
    $('#show-assets')?.addEventListener('click', toggleAssets); $('#open-assets')?.addEventListener('click', toggleAssets);
